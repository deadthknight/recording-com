...
# 开始游戏按钮区域
START_GAME_REGION = (1213, 987, 137, 40)
# 体力补充
...
def detect():
    ...
    # ===================== 【最终版】主界面判断 =====================
    start_text = ocr_text(START_GAME_REGION)
    if "开始" in start_text:
        # 检测体力补充界面
        体力_text = ocr_text(ICON_1_REGION)
        log(f"检测到：开始游戏 | 体力窗口：{repr(体力_text)}")

        # ✅ 你的核心逻辑：同时满足 → 体力耗尽
        if "开始" in start_text and ("体力" in 体力_text or "补充" in 体力_text):
            log("判定：体力耗尽 → 开始退出流程")
